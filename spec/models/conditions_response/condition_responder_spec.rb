...
RSpec.describe ConditionResponder, type: :model do
  ...
  describe '.days_a_date_is_away_from(a_date, timing, some_date)' do
    ...
    context 'timing is before' do
      ...
      it '2nd date is 1 day after the date  = -1' do
        expect(ConditionResponder.days_2nd_date_is_away_from(nov_30, dec_1, timing_before)).to eq(-1)
      end

      it '2nd date is == the date = 0' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_1, dec_1, timing_before)).to eq 0
      end

      it '2nd date is 1 day before  = 1' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_2, dec_1, timing_before)).to eq 1
      end
    ...
    context 'timing is after' do
      ...
      it '2nd date is 1 day after the date = 1' do
        expect(ConditionResponder.days_2nd_date_is_away_from(nov_30, dec_1, timing_after)).to eq(1)
      end

      it '2nd date == the date = 0' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_1, dec_1, timing_after)).to eq 0
      end

      it '2nd date is 1 day before the date = -1' do
        expect(ConditionResponder.days_2nd_date_is_away_from( dec_2, dec_1, timing_after)).to eq(-1)
      end
    ...
    context 'timing is on (always returns 0 days away; this means always check on the 2nd date no matter how many days away)' do
      ...
      it '2nd date is 1 day before the date = 0' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_1, nov_30, timing_on)).to eq 0
      end

      it '2nd date is == the date = 0' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_1, dec_1, timing_on)).to eq 0
      end

      it '2nd date is 1 day after the date = 0' do
        expect(ConditionResponder.days_2nd_date_is_away_from(dec_1, dec_2, timing_on)).to eq 0
      end
  ...
  describe '.days_today_is_away_from(timing, some_date)' do
    ...
    context 'timing is before' do
      ...
      it 'today is 1 day before the date = ConditionResponder.days_2nd_date_is_away_from(Date.current, nov_30, timing_before)' do
        expect(ConditionResponder.days_today_is_away_from(nov_30, timing_before)).to eq ConditionResponder.days_2nd_date_is_away_from(dec_1, nov_30, timing_before)
      end

      it 'today = the date = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_before)' do
        expect(ConditionResponder.days_today_is_away_from(dec_1, timing_before)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_before)
      end

      it 'today is 1 day after the date = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_before)' do
        expect(ConditionResponder.days_today_is_away_from(dec_2, timing_before)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_before)
      end
    ...
    context 'timing is after' do
      ...
      it 'today is 1 day after the date = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_after)' do
        expect(ConditionResponder.days_today_is_away_from(dec_2, timing_after)).to eq  ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_after)
      end

      it 'date is on today = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_after)' do
        expect(ConditionResponder.days_today_is_away_from(dec_1, timing_after)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_after)
      end

      it 'today is 1 day before the date = ConditionResponder.days_2nd_date_is_away_from(Date.current, nov_30, timing_after)' do
        expect(ConditionResponder.days_today_is_away_from(nov_30, timing_after)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, nov_30, timing_after)
      end
    ...
    context 'timing is on (always returns 0 days away; this means always check on today)' do
      ...
      it 'date is 1 day before today = ConditionResponder.days_2nd_date_is_away_from(Date.current, nov_30, timing_on)' do
        expect(ConditionResponder.days_today_is_away_from(nov_30, timing_on)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, nov_30, timing_on)
      end

      it 'date is on today = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_on)' do
        expect(ConditionResponder.days_today_is_away_from(dec_1, timing_on)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_1, timing_on)
      end

      it 'date is 1 day after today = ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_on)' do
        expect(ConditionResponder.days_today_is_away_from(dec_2, timing_on)).to eq ConditionResponder.days_2nd_date_is_away_from(Date.current, dec_2, timing_on)
      end

...
RSpec.describe Address, type: :model do
  ...
  describe 'gecoding' do
    ...
    it 'geocode from address' do
      ...
      expect(addr.latitude.round(3)).to eq(56.7440333.round(3)), addr_details(addr, "expected lat to be 56.744, but wasn't")
      expect(addr.longitude.round(3)).to eq(12.727637.round(3)), addr_details(addr, "expected long to be 12.728, but wasn't")
    end
    ...
    describe 'changed address so update latitude, longitude' do
      ...
      it 'changed street address' do
        ...
        expect(addr.latitude.round(3)).to eq(56.7442343.round(3)), addr_details(addr, "expected lat to be 56.744, but wasn't")
        expect(addr.longitude.round(3)).to eq(12.7255982.round(3)), addr_details(addr, "expected long to be 12.726, but wasn't")
      end
      ...
      it 'changed city' do
        ...
        expect(addr.latitude.round(3)).to eq(56.633333.round(3)), addr_details(addr, "expected lat to be 56.633, but wasn't")
        expect(addr.longitude.round(3)).to eq(13.2.round(3)), addr_details(addr, "expected long to be 13.2, but wasn't")
      end
    ...
    it 'if all info is nil, will at least return lat/long of Sweden' do
      ...
      expect(addr.latitude.round(3)).to eq(60.12816100000001.round(3)), addr_details(addr, "expected lat to be 60.128, but wasn't")
      expect(addr.longitude.round(3)).to eq(18.643501.round(3)), addr_details(addr, "expected long to be 18.644, but wasn't")
    end
    ...
    describe '#geocode_best_possible' do
      ...
      it 'all valid address components' do
        address = Address.new(street_address: 'Matarengivägen 24',
                       ...
                       city: 'Övertorneå')
        address.validate
        expect(address.latitude.round(3)).to eq(66.3902539.round(3)), addr_details(address, "expected lat to be 66.390, but wasn't")
        expect(address.longitude.round(3)).to eq(23.6601303.round(3)), addr_details(address, "expected long to be 23.660, but wasn't")
      end
      ...
      it 'invalid street_address' do
        address = Address.new(street_address: 'blorf',
                          ...
                          city: 'Övertorneå')
        address.validate
        expect(address.latitude.round(3)).to eq(66.3887731.round(3)), addr_details(address, "expected lat to be 66.389, but wasn't")
        expect(address.longitude.round(3)).to eq(23.6734973.round(3)), addr_details(address, "expected long to be 23.673, but wasn't")
      end
      ...
      it 'invalid post_code, street_address' do
        address = Address.new(street_address: 'blorf',
                              ...
                              city: 'Övertorneå')
        address.validate
        expect(address.latitude.round(3)).to eq(66.3884436.round(3)), addr_details(address, "expected lat to be 66.388, but wasn't")
        expect(address.longitude.round(3)).to eq(23.639283.round(3)), addr_details(address, "expected long to be 23.639, but wasn't")
      end
      ...
      it 'invalid city, post_code, street_address' do
        ...
        expect(address.latitude.round(3)).to eq(60.128161.round(3)), addr_details(address, "expected lat to be 60.128, but wasn't")
        expect(address.longitude.round(3)).to eq(18.643501.round(3)), addr_details(address, "expected long to be 18.643, but wasn't")
      end
